...
#include <stdlib.h>
#include <windows.h>
#define UI_WIDTH 20//显示界面的宽度
#define UI_HEIGHT 30 //界面的高度
#define WALL 1
#define BLOCK 2
...
void initia()
{
	...
	for(i = 0; i < UI_HEIGHT; i++)
	{
		for(j = 0; j < UI_WIDTH; j++)
		{
			...
			{game_arr[i][j].info = WALL; game_arr[i][j].var = 1;continue;}  //.var=1表示该点被占用
			if(j == 0 || j == UI_WIDTH-1)  
